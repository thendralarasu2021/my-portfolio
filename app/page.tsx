...
import { 
  ...
  Layers, Globe2, Sparkles, User, X, CheckCircle2 
} from 'lucide-react';
...
const projects = [
  {
    ...
    fullDesc: "This platform empowers students to learn complex technical subjects in their native languages. It features real-time AI-driven translation of course materials and interactive multi-language support. Built to ensure that language is never a barrier to quality education.",
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 selection:bg-emerald-500/30 overflow-x-hidden">
      ...
      <section className="pt-24 pb-12 px-6 max-w-6xl mx-auto text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="flex justify-center gap-8">
            <a href="https://linkedin.com/in/thendral-arasu-s-1b211223a" target="_blank" className="hover:text-blue-400 transition-all hover:scale-125"><Linkedin size={32} /></a>
            <a href="mailto:thenddralkaviya833@gmail.com" className="hover:text-emerald-400 transition-all hover:scale-125"><Mail size={32} /></a>
          </div>
        </motion.div>
      </section>
      ...
      <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-12 mb-20">
        ...
        <div className="md:col-span-1 space-y-10">
          ...
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} className="bg-slate-900/40 p-8 rounded-3xl border border-slate-800 backdrop-blur-xl">
            ...
            <ul className="space-y-2 text-xs text-slate-500">
              <li className="flex items-center gap-2"><CheckCircle2 size={14} className="text-emerald-500"/> Self-motivated</li>
              <li className="flex items-center gap-2"><CheckCircle2 size={14} className="text-emerald-500"/> Task-oriented</li>
            </ul>
          </motion.div>
          ...
          <motion.div whileHover={{ y: -5 }} className="bg-slate-900/40 p-8 rounded-3xl border border-slate-800">
            ...
            <p className="text-blue-300 font-bold leading-tight">Vel Tech High Tech Engineering College</p>
            <p className="text-slate-500 text-xs mt-1">B.Tech Information Technology</p>
            <div className="mt-6 flex items-baseline gap-2">
              <span className="text-3xl font-black text-white">8.27</span>
              <span className="text-slate-500 text-xs">AGGREGATE CGPA</span>
            </div>
          </motion.div>
        </div>
        ...
        <div className="md:col-span-2">
          <div className="flex items-center gap-4 mb-8"><Layers className="text-cyan-400" /> <h2 className="text-2xl font-bold">Featured Projects</h2></div>
            ...
            {projects.map((project, index) => (
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {project.tags.slice(0, 3).map(t => <span key={t} className="text-[10px] bg-slate-800 text-slate-300 px-3 py-1 rounded-full border border-slate-700">{t}</span>)}
                  <span className="text-[10px] bg-emerald-500/10 text-emerald-400 px-3 py-1 rounded-full border border-emerald-500/20">View Details</span>
                </div>
              </motion.div>
            ))}
          </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-950/80 backdrop-blur-md">
            ...
            >
              ...
              <div className="p-10 -mt-16">
                <div className={`inline-block p-4 rounded-3xl bg-slate-900 border border-slate-800 mb-6 shadow-xl`}>
                ...
                <h4 className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-4">Key Features</h4>
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedProject.tags.map(t => <span key={t} className="text-xs bg-slate-800 text-slate-400 px-4 py-2 rounded-xl">{t}</span>)}
                </div>
              </div>
            </motion.div>
          </div>
